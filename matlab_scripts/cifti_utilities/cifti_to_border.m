...
% (the script will draw borders around the blobs). Set filled to false for
% the former; set it to true for the latter. [default = true]
...
% Set allcolors_inonemap to false for the former; set it to true for the
% latter. [default = true]
%bordercolors - you can 
% a) set bordercolors to be an rgb triple; 
% b) set bordercolors to be an N by 3  matrix of N rgb triples representing
% appropriate colors for the N maps / N discrete values in the cifti; or 
% c) set bordercolors to 'power', which attempts to map colors to the
% standard values for the power_surf pallete. With the 'power' option, any
% values that are outside 1 and 17 will be set to "Unknown" and colored
% gray; any values that are non-integers between 1 and 17 will be set to
% "Unknown" and given an intermediate color appropriate for the power_surf
% pallete. [default = 'power']
%
%E. Gordon 07/10/16
...
if ~exist('allcolors_inonemap','var')
    allcolors_inonemap = true; 
end


if ~exist('bordercolors','var')
    bordercolors = 'power';
end
...
%IDs = IDs([find(~logical(mod(IDs,1))); 
...
if ischar(bordercolors) && strcmp(bordercolors,'power') 
    defaultcolors = [1 0 0;0 0 .6;.9 .9 0;1 .7 .4;0 .8 0;1 .6 1;0 .6 .6;0 0 0;.3 0 .6;.2 1 1;1 .5 0;.6 .2 1;0 .2 .4;.2 1 .2;0 0 1;.85 .85 .85;0 .5 0;.2 .2 .2;.4 .4 .4];
    default_colnames = {'Default','Visual','Fronto-Parietal','PrimaryVisual','DorsalAttention','PreMotor','VentralAttentionLanguage','Salience','Cingulo-Opercular','MotorHand','MotorMouth','Auditory','AntMedialTemporal','PostMedialTemporal','Cingulo-Parietal','Parieto-Occipital','MotorFoot','Other','Unknown'};
    
    bordercolors = [];
    colnames_bothhem = cell(0,1);
    for IDnum = 1:length(IDs)
        thisnetwork = IDs(IDnum);
        if thisnetwork<1 || thisnetwork > 18
            bordercolors(IDnum,:) = defaultcolors(19,:);
            colnames_bothhem(IDnum) = {['Unknown network ID ' num2str(thisnetwork)]};
        else
            base = floor(thisnetwork);
            factor = thisnetwork-base;
            bordercolors(IDnum,:) = (defaultcolors(base,:) .* (1-factor)) + (defaultcolors(base+1,:) .* factor);
            if ~logical(mod(thisnetwork,1))
                colnames_bothhem(IDnum) = default_colnames(thisnetwork);
            else
                colnames_bothhem(IDnum) = {['Unknown network ID ' num2str(thisnetwork)]};
            end
        end
...
MNI{1} = gifti(['/data/cn/data1/scripts/CIFTI_RELATED/Resources/Conte69_atlas-v2.LR.32k_fs_LR.wb/Conte69.L.midthickness.32k_fs_LR.surf.gii']); MNI{1} = MNI{1}.vertices;
MNI{2} = gifti(['/data/cn/data1/scripts/CIFTI_RELATED/Resources/Conte69_atlas-v2.LR.32k_fs_LR.wb/Conte69.R.midthickness.32k_fs_LR.surf.gii']); MNI{2} = MNI{2}.vertices;
...
for hem = 1:2
    ...
    if isempty(extensionloc)
        ...
        if isempty(extensionloc)
            extensionloc = strfind(ciftifile,'.dlabel.nii');
        end
    end
