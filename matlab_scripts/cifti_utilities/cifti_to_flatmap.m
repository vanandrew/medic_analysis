function cifti_to_flatmap(cifti_filename, anatomical_inciftispace,discrete)
%cifti_to_flatmap(cifti_filename, anatomical_inciftispace,[discrete])
%
%Map cifti cerebellar data onto a cerebellar flatmap (from the SUIT
% toolbox) that can be viewed within workbench.
%
%Inputs:
%'cifti_filename' - a cifti with cerebellar data that will be mapped onto
% a flatmap
%'anatomical_inciftispace' - a T1 image in the same volumetric space as the
% cifti file.
%'discrete' - an optional true/false argument specifying whether values are
% discrete (e.g., network values) or continuous. True=discrete;
% false=continuous. Default: false.
%
%E. Gordon 2021


warning off
dotinds = strfind(cifti_filename,'.');
basename = [cifti_filename(1:(dotinds(end-1)-1)) '_temp4flatmap'];
filename = cifti_filename;
...
save(gifti(single(Data)),[cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.func.gii'])
system(['wb_command -set-structure ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.func.gii CEREBELLUM'])

system(['wb_command -add-to-spec-file ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.spec CEREBELLUM /data/nil-bluearc/GMT/Evan/Scripts/suit/flatmap/FLAT.surf.gii'])
system(['wb_command -add-to-spec-file ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.spec CEREBELLUM /data/nil-bluearc/GMT/Evan/Scripts/suit/flatmap/SUIT.shape.gii'])
system(['wb_command -add-to-spec-file ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.spec CEREBELLUM /data/nil-bluearc/GMT/Evan/Scripts/suit/flatmap/fissures.border'])
system(['wb_command -add-to-spec-file ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.spec CEREBELLUM ' cifti_filename(1:(dotinds(end-1)-1)) '_cerebellumflat.func.gii'])
