function wb_surfer_wrapper(ciftifile,borderfile,outname,varargin)
%wb_surfer_wrapper(ciftifile,borderfile,outname,varargin)
...
% Mandatory inputs:
%
%  ciftifile - FULL PATH of dconn file
%
...
[ciftipathpath,~,~] = fileparts(ciftifile);
if isempty(ciftipathpath)
    ciftifile = [pwd '/' ciftifile];
end
system(['ln -s ' ciftifile ' ' capturefolder '/Corr.dconn.nii']) %copyfile(ciftifile,[capturefolder '/Corr.dconn.nii'])
