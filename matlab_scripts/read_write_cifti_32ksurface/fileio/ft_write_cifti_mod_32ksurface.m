function ft_write_cifti_mod_32ksurface(filename, source, varargin)
...
%
% $Id: ft_write_cifti.m 9919 2014-10-19 09:29:16Z roboos $

parameter       = 'data';%ft_getopt(varargin, 'parameter');
...
dimord = getdimord_mod(source, parameter);
...
[p, f, x] = fileparts(filename);
if any(strcmp(x, {'.dtseries', '.ptseries', '.dconn', '.pconn', '.dscalar', '.pscalar'}))
  filename = fullfile(p, f); % strip the extension
end
...
if any(strcmp(dimtok, 'time'))
  ...
  if (~isfield(source,'time')) || (length(source.time) ~= size(dat,1))
      source.time = [1:size(dat,1)];
  end
...
if any(strcmp(dimtok, 'scalar'))
  ...
  tree = attributes(tree, 'add', branch, 'IndicesMapToDataType', 'CIFTI_INDEX_TYPE_SCALARS');
  source.mapname = source.mapname(1:size(source.data,2));
  for i=1:length(source.mapname)
      branch = find(tree, 'CIFTI/Matrix/MatrixIndicesMap');
      branch = branch(end);
      ...
      tree        = add(tree, uid, 'chardata', source.mapname{i});
  end
...
if any(strcmp(dimtok, 'pos'))
  ...
  for i=1:length(BrainStructurelabel)
    ...
    if i == 1
        ...
        IndexOffset = IndexOffset + length(find(sel)); %find(sel, 1, 'first') - 1; % zero offset
    end
    ...
    if all(ModelType(sel)==find(strcmp(ModelTypelabel, 'VOXEL')))
      ...
      tmp = find(sel)-SurfaceNumber-1; % zero offset
      SurfaceNumber = 32492;
      tree = attributes(tree, 'add', branch, 'IndexOffset', printwithspace(IndexOffset));
      tree = attributes(tree, 'add', branch, 'IndexCount', printwithspace(IndexCount));
      tree = attributes(tree, 'add', branch, 'ModelType', 'CIFTI_MODEL_TYPE_SURFACE');
      tree = attributes(tree, 'add', branch, 'BrainStructure', BrainStructurelabel{i});
      tree = attributes(tree, 'add', branch, 'SurfaceNumberOfVertices', printwithspace(SurfaceNumber));
...
if any(strcmp(dimtok, 'chan'))
  ...
  for i=1:length(BrainStructurelabel)
      ...
      if any(strcmp(BrainStructurelabel{i},{'CIFTI_STRUCTURE_CORTEX_LEFT','CIFTI_STRUCTURE_CORTEX_RIGHT'}))
          
          [tree, uid] = add(tree, branch, 'element', 'Surface');
          tree        = attributes(tree, 'add', uid, 'BrainStructure', BrainStructurelabel{i});
          tree        = attributes(tree, 'add', uid, 'SurfaceNumberOfVertices', printwithspace(max(find(source.brainstructure==i))-min(find(source.brainstructure==i))+1));
...
switch dimord
  case 'pos_scalar'
    hdr.dim             = [6 1 1 1 1 size(source.data,1)  size(source.data,2)   1]; % only single scalar
  case 'scalar_pos'
    hdr.dim             = [6 1 1 1 1 size(source.data,2)  size(source.data,1)   1]; % only single scalar
  case 'chan_scalar'
    hdr.dim             = [6 1 1 1 1 numel(source.label)  1                    1]; % only single scalar
  case 'scalar_chan'
    hdr.dim             = [6 1 1 1 1 1                    numel(source.label)  1]; % only single scalar
  case 'pos_time'
    hdr.dim             = [6 1 1 1 1 size(source.data,1)   length(source.time)  1];
  case 'time_pos'
    hdr.dim             = [6 1 1 1 1 length(source.time)  size(source.data,1)   1];
  case 'chan_time'
    hdr.dim             = [6 1 1 1 1 length(source.label) length(source.time)  1];
  case 'time_chan'
    hdr.dim             = [6 1 1 1 1 length(source.time)  length(source.label) 1];
  case 'pos_pos'
    hdr.dim             = [6 1 1 1 1 size(source.data,1)   size(source.data,1)   1];
  case 'chan_chan'
  ...
  case 'pos_pos_time'
    hdr.dim             = [6 1 1 1 1 size(source.data,1)   size(source.data,1)   length(source.time)];
  case 'chan_chan_freq'
    hdr.dim             = [6 1 1 1 1 length(source.label) length(source.label) length(source.freq)];
  case 'pos_pos_freq'
    hdr.dim             = [6 1 1 1 1 size(source.data,1)   size(source.data,1)   length(source.freq)];
  otherwise
    error('unsupported dimord "%s"', dimord)
end % switch
